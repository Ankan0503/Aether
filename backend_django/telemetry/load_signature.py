...
Shared with backend/load_signature.py in this repo - the bench tooling and the
server must classify identically, or a signature captured over serial would not
match one that arrived over MQTT.
...
DEFAULT_MODEL_DIR = Path(__file__).resolve().parent.parent / "anomaly" / "models"
...
def classify_by_rule(features: dict[str, float]) -> tuple[str, float, str]:
    ...
    return CLASS_MIXED, 0.4, (
        f"between a sine and a switching supply (crest {crest:.2f}, THD {thd:.2f}) - ""typical of an active-PFC supply or two loads sharing one socket"
    )
